...
class KRNXController:
                ...
                if should_flush and pending_events:
                    try:
                        ...
                        if pending_msg_ids:
                            ...
                            # FIX: Trim processed messages so XLEN reflects actual backlog
                            # This prevents backpressure from triggering on already-processed messages
                            try:
                                redis_client.xtrim('krnx:ltm:queue', minid=pending_msg_ids[-1])
                            except Exception:
                                pass  # Non-critical, just optimization
                        ...
                        for i, event in enumerate(pending_events):
                            try:
                                ...
                                last_successful_id = pending_msg_ids[i]
                                self._worker_metrics['messages_processed'] += 1
                            except Exception as e:
                                logger.error(f"[FAIL] Event {event.event_id}: {e}")
                        # Trim up to last successful message
                        if last_successful_id:
                            try:
                                redis_client.xtrim('krnx:ltm:queue', minid=last_successful_id)
                            except Exception:
                                pass
                ...
                if events_to_store:
                    try:
                        ...
                        if msg_ids_to_ack:
                            ...
                            try:
                                redis_client.xtrim('krnx:ltm:queue', minid=msg_ids_to_ack[-1])
                            except Exception:
                                pass
    ...
    def get_worker_metrics(self) -> WorkerMetrics:
        ...
        try:
            ...
            # Estimate lag from OLDEST unprocessed message (first-entry)
            # FIX: Was using last-entry which measures "time since last write"
            # We want "how old is the oldest message waiting to be processed"
            lag_seconds = 0.0
            if queue_len > 0:
            ...
            # If queue is empty, lag is 0 (no backlog)
        except Exception:
            queue_len = 0
            lag_seconds = 0.0
