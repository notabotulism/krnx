"""
KRNX Controller - Pure Kernel v0.3.1
...
- Worker batches on read (XREADGROUP count=500)
...
class KRNXController:
    ...
    def _ltm_worker_loop(self):
        """
        LTM worker loop (Industry Standard Pattern).
        
        CONSUMER:
        - Batches on read: XREADGROUP count=500
        - Batches on write: SQLite batch insert
        - ACKs after successful write
        
        This is the RIGHT way to build producer-consumer:
        - Producer writes fast (XADD)
        - Consumer batches naturally (XREADGROUP + batch insert)
        - No stuck events, no timers, no complexity
        """
        redis_client = get_redis_client()
        worker_id = f"worker-{int(time.time())}"
        
        logger.info(f"[WORKER] LTM worker '{worker_id}' started")
        logger.info(f"   Batch size: {self.worker_batch_size} events per read")
        logger.info(f"   Block time: {self.worker_block_ms}ms")
        
        while self._worker_running:
            try:
                # BATCH READ: Get up to N events at once
                messages = redis_client.xreadgroup(
                    groupname='krnx-ltm-workers',
                    consumername=worker_id,
                    streams={'krnx:ltm:queue': '>'},
                    count=self.worker_batch_size,
                ...
                for stream_name, msg_list in messages:
                    for msg_id, data in msg_list:
                        try:
                            ...
                            # Skip bad message but don't crash worker
                
                # BATCH WRITE: Store all events in one transaction
                if events_to_store:
                    try:
                        stored_count = self.ltm.store_events_batch(events_to_store)
                        # ACK after successful store
                        if msg_ids_to_ack:
                        ...
                        logger.warning(f"[FALLBACK] Batch failed ({batch_error}), trying individual inserts")
                        # FALLBACK: Try each event individually
                        for i, event in enumerate(events_to_store):
                            try:
                                self.ltm.store_event(event)
                                # ACK this one
                                redis_client.xack('krnx:ltm:queue', 'krnx-ltm-workers', msg_ids_to_ack[i])
                                self._worker_metrics['messages_processed'] += 1
                            except Exception as e:
                                logger.error(f"[FAIL] Event {event.event_id} failed: {e}")
                                self._error_tracker.record_error(str(e))
        ...
        # DRAIN: Process remaining pending messages before exit
        logger.info(f"[DRAIN] Worker draining pending messages...")
        try:
            # Check if LTM is still accessible
            if not self.ltm.db:
                logger.info("[DRAIN] Skipping drain - LTM already closed")
                return
            ...
            logger.error(f"[DRAIN] Drain failed: {e}")
