...
class KRNXController:
    ...
    def erase_user(self, workspace_id: str, user_id: str) -> Dict[str, Any]:
        ...
        # Delete from LTM (uses thread-safe delete method with write lock)
        ltm_result = self.ltm.delete_user_events(workspace_id, user_id)
        ltm_warm_deleted = ltm_result['warm_deleted']
        ltm_cold_deleted = ltm_result['cold_deleted']
    ...
    def erase_workspace(self, workspace_id: str) -> Dict[str, Any]:
        ...
        # Delete from LTM (uses thread-safe delete method with write lock)
        ltm_result = self.ltm.delete_workspace_events(workspace_id)
        ltm_warm_deleted = ltm_result['warm_deleted']
        ltm_cold_deleted = ltm_result['cold_deleted']
