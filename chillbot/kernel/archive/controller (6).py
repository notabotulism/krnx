"""
KRNX Controller - Pure Kernel v0.3.3 TURBO

PERFORMANCE FIX: Single-pipeline writes
Before: 3 round-trips per event (~3ms + contention)
After:  1 round-trip per event (~1ms, minimal contention)

Changes from v0.3.2:
- write_event_turbo() combines STM + LTM queue + job queue in ONE pipeline
- 3x throughput improvement under concurrency
"""
...
class KRNXController:
    """
    KRNX Kernel Controller v0.3.3 TURBO
    
    PERFORMANCE: Single-pipeline writes for 3x throughput.
    """
    ...
    def write_event_turbo(
        ...
        Returns:
            Stream message ID
        """
        # Lazy backpressure check
        self._write_count += 1
        if self.enable_backpressure and self._write_count % 100 == 0:
            metrics = self.get_worker_metrics()
            if metrics.queue_depth > self.max_queue_depth or metrics.lag_seconds > self.max_lag_seconds:
                self._backpressure_mode = True
            else:
                self._backpressure_mode = False
        
        if self._backpressure_mode:
            raise BackpressureError(f"System under load")
