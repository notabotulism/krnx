...
class KRNXController:
    ...
    def write_event_turbo(
        ...
        if self.enable_backpressure:
            ...
            # Only check backpressure every N ms (time-based, not count-based)
            # This avoids expensive Redis calls on every write
            with self._backpressure_lock:
                should_check = (now - self._last_backpressure_check) > self._backpressure_check_interval
                current_mode = self._backpressure_mode
                ...
                with self._backpressure_lock:
                    ...
                    if (now - self._last_backpressure_check) > self._backpressure_check_interval:
                        try:
                            ...
                            # If we can't check, don't enable backpressure
                            pass
                        current_mode = self._backpressure_mode
